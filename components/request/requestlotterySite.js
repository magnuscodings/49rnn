      ...
      try {
        ...
        if(result.data.recordList.length >0){
          ...
          recordList.forEach((record,index) => {
            var id = result.data.recordList[index].lotteryRecordId.toString()

            LiveImage.push({
              lotteryid: id,
              period: result.data.recordList[index].period,
              year: result.data.recordList[index].year,
...
const LastRecordLink = (livelink) => {
// const lastRecordlink = "https://h5.49217006.com:8443/unite49/h5/index/lastLotteryRecord?lotteryType=1";
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(livelink.livelink);
        const result = await response.json();
        
        lastRecordLinks =[];
        result.data.recommendList.forEach((record,index) => {
           lastRecordLinks.push(result.data.recommendList[index])
        })
    ...
    fetchData();
  }, []); // Make sure to pass an empty s effect should only run once
};
...
export const Request_BallsLottery = ({ apiUrl, keyname, key ,link ,livelink}) => {
  // console.log(link)
...
const FooterComponent = () => {
  console.log(lastRecordLinks)
  return (
    <View>
      {lastRecordLinks.map((item, index) => (
        <View key={index}>
          <View style={styles.card}>
            <Text style={[styles.cardText, styles.bgCard]}>
              {item.year}  
              <Text style={styles.greenText}>第</Text> 
              {item.period}
              <Text style={styles.greenText}>推荐</Text> 
            </Text>

            {item.detailList.map((detail, index) => {
              // Conditionally apply styles based on detail.name
              const textStyle = detail.name === '十码' ? styles.greenText2 : styles.cardText;

              return (
                <Text key={index} style={textStyle}>
                  {detail.name} : {' '}
                  {detail.valueList.map((value, idx) => (
...
<LastRecordLink livelink={livelink}/>
