...
export const Request_Balls = ({ apiUrl, keyname, period,nextperiod,year  }) => {
  const requestBallsRef = useRef('request'); // Create a ref for the Request_Balls component
  const yourComponentRef = useRef(null); // This is your references
 const data = apiUrl;
  return (
    <>
 ...
 <View ref={requestBallsRef}>
      <View style={StyleBall.countdownContainer}>
        <Text style={StyleBall.countdownTextBlack}> 第<Text style={StyleBall.countdownTextGreen}>{period}</Text> 期</Text>
    ...
    {data.map((item, index) => {
      ...
      <View style={StyleBall.cameraContainer}>
          <Text style={[StyleBall.cameraTextRed]}>{nextperiod}</Text>
        </View>
    </>
  );
};
